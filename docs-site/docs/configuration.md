...
**Diagnostics bundle.** _Help > Save Diagnostics..._ writes a JSON file
...
A `managedPolicy` section in a _user_ config file is ignored and discarded, so
...
```json
{
  "electronCLIFlags": ["disable-software-rasterizer"]
}
```
