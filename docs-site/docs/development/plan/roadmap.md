...
Passkey sign-in: Linux has no platform authenticator, so Entra's passkey page stalled with no error. A page-world shim now wraps `navigator.credentials` and hands the ceremony to the main process, which binds it to the requesting frame's origin - the frame rather than the window, because Entra runs the ceremony in an iframe - and requires the page-supplied `rpId` to be that host or a registrable parent of it. The page contributes only the challenge and the credential filters, so it cannot have a credential signed for a relying party it does not control. Arca is the first backend; `passkey.enabled` gates the integration in the main process, and every failure falls back to Chromium's own handler, so a machine with no provider behaves exactly as before.

Remaining: `contextIsolation` is still `false`. [ADR-020](../adr/020-context-isolation-migration.md) documents why --- `ReactHandler` reads a main-world React expando that an isolated world cannot see, and 12 of 16 browser tools depend on main-world execution.

Progress against the ADR's six stages: stage 1 (page-exposed `electronAPI` cut from 22 entries to 2) and stage 2 (the main-world bridge, shipped inert with 67 tests) are done. Stage 3 is partial --- `disableAutogain`, `cameraResolution` and `cameraAspectRatio` now run in the page world. Two items originally scoped into stage 3 moved: `speakingIndicator` turned out to import `activityHub`, which imports `reactHandler`, so it belongs to stage 4; and the Notification override needs bridge callbacks for sound and toast delivery, so it is its own change and the riskiest part of the stage.

Stage 5 has since acquired a hard blocker of its own. The passkey shim must wrap `navigator.credentials` before any page script runs, which the preload's top level achieves only while `contextIsolation` is `false`. The bridge agent injects on `DOMContentLoaded`, too late to take over, so that stage now needs a pre-script injection point rather than a relocation of the shim.

The blocker on stage 4 has shifted. It is no longer that the tests do not exist --- the authenticated suite went from 6 to 20, and `integration-surface.spec.js` exists specifically to make these failures loud. It is that nobody has **run** them: the suite needs `E2E_SESSION_DIR` pointing at a logged-in session and does not run in CI, so it requires someone with a tenant to execute `npm run test:authenticated`. Until that happens, stage 4 would be shipping changes to Teams' undocumented internals with no verification at all.

One note for whoever picks up stage 4: the ReactHandler assertion in `integration-surface.spec.js` is deliberately written against the current pre-migration arrangement. It must be rewritten to drive the handler through the bridge, **not deleted** --- deleting it removes the only automated check that Teams' internals are reachable at all.
