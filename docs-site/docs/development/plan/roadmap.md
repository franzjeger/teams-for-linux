...
Remaining: `contextIsolation` is still `false`. [ADR-020](../adr/020-context-isolation-migration.md) documents why - `ReactHandler` reads a main-world React expando that an isolated world cannot see, and 12 of 16 browser tools depend on main-world execution. Stage 1 (shrinking the page-exposed `electronAPI` from 22 entries to 2) and stage 2 (the main-world bridge) have shipped. Stage 5 has since acquired a hard blocker: the passkey shim must wrap `navigator.credentials` before any page script, which the preload's top level achieves only while `contextIsolation` is `false`. The bridge agent injects on `DOMContentLoaded`, too late to take over, so that stage now needs a pre-script injection point rather than a relocation. Stages 3 and 4 still need the authenticated test suite expanded first, which overlaps with the Phase 1 testing work below.
