...
function loadChangelogEntries() {
  ...
  const entries = files.map((file) => {
    ...
    const content = fs
      .readFileSync(path.join(changelogDir, file), "utf8")
      .trim()
      .split(/\r?\n/)[0]
      .trim();
    console.log(`   • ${content}`);
...
function showNextSteps(newVersion) {
  ...
  console.log(
    `   gh pr create --title "Release v${newVersion}" --body-file <(npm run generate-release-notes ${newVersion})`,
  );
}
...
async function main() {
  if (dryRun) {
    ...
    console.log(
      "║         No files will be modified. Preview only.           ║",
    );
    console.log(
      "╚════════════════════════════════════════════════════════════╝\n",
    );
  ...
  const { updatedXml, appdataPath, releaseDate } = await prepareAppdataRelease(
    entries,
    newVersion,
  );
  ...
  if (dryRun) {
    ...
    showReleaseNotesPreview(
      newVersion,
      "📋 RELEASE NOTES PREVIEW (generated from current changelog)",
    );
