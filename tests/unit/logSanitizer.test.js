...
function runContainsTests(cases) {
  cases.forEach(([name, input, shouldContain, shouldNotContain = []]) => {
    test(name, () => {
      ...
      shouldNotContain.forEach((s) =>
        assert.ok(!result.includes(s), `Should not contain "${s}": ${result}`),
      );
    });
  });
}
...
describe("Email sanitization", () => {
  runExactTests([
    ...
    [
      "multiple emails",
      "From: sender@example.com To: receiver@company.org",
      "From: [EMAIL] To: [EMAIL]",
    ],
    [
      "subdomain email",
      "Contact: admin@mail.subdomain.example.co.uk",
      "Contact: [EMAIL]",
    ],
...
describe("UUID sanitization", () => {
  runExactTests([
    ...
    [
      "uppercase UUIDs",
      "Session: ABCDEFAB-1234-5678-90AB-CDEF12345678",
      "Session: ABCDEFAB...",
    ],
    [
      "multiple UUIDs",
      "User 11111111-2222-3333-4444-555555555555 joined 66666666-7777-8888-9999-aaaaaaaaaaaa",
      "User 11111111... joined 66666666...",
    ],
  ]);
});
...
describe("Bearer token sanitization", () => {
  ...
  runExactTests([["lowercase bearer", "bearer abc123xyz", "Bearer [TOKEN]"]]);
});
...
describe("IP address sanitization", () => {
  runExactTests([
    ...
    [
      "multiple IPs",
      "Route: 10.0.0.1 -> 172.16.0.1 -> 8.8.8.8",
      "Route: [IP] -> [IP] -> [IP]",
    ],
  ]);
  runContainsTests([["localhost IP", "Listening on 127.0.0.1:3000", ["[IP]"]]]);
});
...
describe("MQTT URL sanitization", () => {
  runContainsTests([
    [
      "mqtt:// credentials",
      "Connecting to mqtt://username:password@broker.example.com:1883",
      ["mqtt://[CREDENTIALS]@"],
      ["username", "password"],
    ],
    [
      "mqtts:// credentials",
      "Connecting to mqtts://user:pass@secure-broker.com",
      ["mqtts://[CREDENTIALS]@"],
    ],
  ]);
});
...
describe("URL query parameter sanitization", () => {
  runContainsTests([
    [
      "redacts query params",
      "Redirect to https://example.com/auth?token=abc123&session=xyz",
      ["?[PARAMS]"],
      ["token="],
    ],
    [
      "preserves path",
      "URL: https://api.example.com/v1/users?api_key=secret",
      ["/v1/users", "?[PARAMS]"],
    ],
  ]);
});
...
describe("API key sanitization", () => {
  runContainsTests([
    [
      "api_key= format",
      "Request with api_key=sk_live_abc123xyz",
      ["api_key=[REDACTED]"],
    ],
    [
      "x-api-key: format",
...
describe("Client secret sanitization", () => {
  runContainsTests([
    [
      "client_secret",
      "OAuth config: client_secret=super_secret_value",
...
describe("Certificate fingerprint sanitization", () => {
  runContainsTests([
    [
      "SHA1 fingerprint",
      "Certificate sha1=AA:BB:CC:DD:EE:FF:00:11:22:33:44:55:66:77:88:99:AA:BB:CC:DD",
      ["[FINGERPRINT]"],
    ],
    [
      "fingerprint= format",
...
describe("User path sanitization", () => {
  runExactTests([
    [
      "Linux home paths",
      "Config at /home/johndoe/.config/teams",
      "Config at /home/[USER]/.config/teams",
    ],
    [
      "macOS user paths",
      "File: /Users/johnsmith/Documents/file.txt",
      "File: /Users/[USER]/Documents/file.txt",
    ],
    [
      "Windows user paths",
      String.raw`Path: C:\Users\JohnDoe\AppData\Local`,
      String.raw`Path: C:\Users\[USER]\AppData\Local`,
    ],
  ]);
});
...
describe("containsPII function", () => {
  ...
  test("false for clean message", () =>
    assert.strictEqual(containsPII("Application started successfully"), false));
  test("false for status message", () =>
    assert.strictEqual(
      containsPII("Status: connected, retry count: 3"),
      false,
    ));
});
...
describe("detectPIITypes function", () => {
  test("detects email", () =>
    assert.ok(detectPIITypes("Email: test@example.com").includes("email")));
  ...
  test("empty for clean message", () =>
    assert.strictEqual(detectPIITypes("Clean log message").length, 0));
});
...
describe("createSanitizer function", () => {
  test("custom patterns", () => {
    ...
    assert.strictEqual(
      custom("Processing INTERNAL-12345"),
      "Processing [INTERNAL_ID]",
    );
  });
...
describe("Edge cases", () => {
  test("multiple PII types", () => {
    const result = sanitize(
      "User john@example.com (ID: 12345678-1234-1234-1234-123456789abc) from 10.0.0.1",
    );
    assert.ok(
      result.includes("[EMAIL]") &&
        result.includes("12345678...") &&
        result.includes("[IP]"),
    );
  });
  test("JSON-like messages", () => {
    const result = sanitize(
      '{"user":"admin@company.com","password":"secret123","ip":"192.168.1.50"}',
    );
    assert.ok(
      result.includes("[EMAIL]") &&
  ...
  test("preserves non-PII", () =>
    assert.strictEqual(
      sanitize("App version 2.7.2 started"),
      "App version 2.7.2 started",
    ));
  test("empty string", () => assert.strictEqual(sanitize(""), ""));
  test("long email", () =>
    assert.ok(
      sanitize(`Long: ${"a".repeat(50)}@${"b".repeat(50)}.com`).includes(
        "[EMAIL]",
      ),
    ));
});
...
describe("Teams for Linux scenarios", () => {
  runContainsTests([
    [
      "MQTT broker",
      "Connecting to: mqtt://teams-user:mqttpass123@broker.home.local:1883",
      ["mqtt://[CREDENTIALS]@"],
      ["teams-user", "mqttpass123"],
    ],
    [
      "Graph API params",
      "Graph API: https://graph.microsoft.com/v1.0/me?$select=mail&access_token=eyJ",
      ["?[PARAMS]"],
    ],
    [
      "SSO credential",
      "SSO for user@company.onmicrosoft.com with access_token=abc123",
      ["[EMAIL]", "access_token=[REDACTED]"],
    ],
    [
      "certificate",
      "Verified: fingerprint=AA:BB:CC:DD:EE:FF:00:11:22:33:44:55:66:77:88:99:AA:BB:CC:DD",
      ["[FINGERPRINT]"],
    ],
    [
      "config path",
      "Loading from /home/realusername/.config/teams-for-linux/config.json",
      ["/home/[USER]"],
    ],
  ]);
});
...
describe("sanitizeObject sensitive key handling", () => {
  ...
  test("redacts nested password key", () => {
    const result = sanitizeObject({
      user: { name: "admin", password: "fake-password-123" },
    });
  ...
  test("redacts password in array of objects", () => {
    const result = sanitizeObject([
      { type: "user", password: "pass1" },
      { type: "admin", password: "pass2" },
    ]);
  ...
  test("redacts token in array of objects", () => {
    const result = sanitizeObject([
      { name: "user1", access_token: "token1" },
