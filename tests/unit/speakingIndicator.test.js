...
const speakingIndicatorPath =
  require.resolve("../../app/browser/tools/speakingIndicator");
const activityHubPath = require.resolve("../../app/browser/tools/activityHub");

function makeStatsReport(audioLevel) {
  const entries = [{ type: "media-source", kind: "audio", audioLevel }];
...
describe("SpeakingIndicator", () => {
  ...
  it("registers call lifecycle events on activityHub", () => {
    ...
    const events = new Set(
      mockActivityHub.on.mock.calls.map((call) => call.arguments[0]),
    );
    assert.ok(
      events.has("call-connected"),
      "should register call-connected handler",
    );
    assert.ok(
      events.has("call-disconnected"),
      "should register call-disconnected handler",
    );
  });
  ...
  it("captures RTCPeerConnection instances when patched", () => {
    ...
    assert.ok(
      pc,
      "RTCPeerConnection should still be constructable after patching",
    );
  });
