...
test("resolveRpId defaults to the frame host when the page names nothing", () => {
  assert.equal(
    resolveRpId("https://login.microsoft.com", undefined),
    "login.microsoft.com",
  );
  assert.equal(
    resolveRpId("https://login.microsoft.com", null),
    "login.microsoft.com",
  );
  assert.equal(
    resolveRpId("https://login.microsoft.com", ""),
    "login.microsoft.com",
  );
});
...
test("resolveRpId accepts the host itself, case-insensitively", () => {
  assert.equal(
    resolveRpId("https://Login.Microsoft.com", "login.microsoft.com"),
    "login.microsoft.com",
  );
  assert.equal(
    resolveRpId("https://login.microsoft.com", "LOGIN.MICROSOFT.COM"),
...
test("resolveRpId accepts a registrable parent domain", () => {
  assert.equal(
    resolveRpId("https://login.microsoft.com", "microsoft.com"),
    "microsoft.com",
  );
});
...
test("resolveRpId rejects a bare public suffix", () => {
  assert.throws(
    () => resolveRpId("https://login.microsoft.com", "com"),
    /not valid/,
  );
});
...
test("resolveRpId requires a secure origin, with localhost excepted", () => {
  assert.throws(
    () => resolveRpId("http://login.microsoft.com"),
    /secure origin/,
  );
...
test("resolveRpId rejects a non-URL origin and a non-string rpId", () => {
  ...
  assert.throws(
    () => resolveRpId("https://login.microsoft.com", 42),
    /must be a string/,
  );
});
...
test("buildClientData rejects an unknown type and an empty challenge", () => {
  assert.throws(
    () => buildClientData("webauthn.sign", [1], "https://x.example"),
    /unknown ceremony/,
  );
  assert.throws(
    () => buildClientData("webauthn.get", [], "https://x.example"),
    /challenge is required/,
  );
  assert.throws(
    () => buildClientData("webauthn.get", null, "https://x.example"),
    /challenge is required/,
  );
});
