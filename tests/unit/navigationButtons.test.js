...
function createDocumentStub() {
  ...
  return {
    ...
    querySelector: (selector) =>
      selector === '[data-tid="search-f6-navigation-region"]'
        ? searchRegion
        : null,
...
function loadModule() {
  delete require.cache[
    require.resolve("../../app/browser/tools/navigationButtons")
  ];
...
describe("NavigationButtons - IPC wiring", () => {
  ...
  it("requests navigation state with the get-navigation-state channel", () => {
    ...
    assert.ok(
      ipc.invoked.some((entry) => entry.channel === "get-navigation-state"),
    );
  });
