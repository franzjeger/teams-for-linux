...
describe("Bridge protocol - createEnvelope", () => {
  ...
  it("rejects a missing session id, unknown kind or empty channel", () => {
    assert.throws(
      () => createEnvelope({ sessionId: "", kind: KIND_EVENT, channel: "c" }),
      /sessionId/,
    );
    assert.throws(
      () => createEnvelope({ sessionId: SESSION, kind: "evil", channel: "c" }),
      /unknown kind/,
    );
    assert.throws(
      () =>
        createEnvelope({ sessionId: SESSION, kind: KIND_EVENT, channel: "" }),
      /channel/,
    );
  });
});
...
describe("Bridge protocol - validateInbound rejects non-bridge traffic", () => {
  ...
  it("rejects a message with a different or absent session id", () => {
    assert.strictEqual(
      validateInbound(envelope({ [MARKER]: "other" }), options()).ok,
      false,
    );
...
describe("Bridge protocol - validateInbound enforces direction", () => {
  ...
  it("rejects unknown kinds", () => {
    for (const kind of ["evil", "", null, 42, undefined]) {
      assert.strictEqual(
        validateInbound(envelope({ kind }), options()).ok,
        false,
      );
    }
  });
...
describe("Bridge protocol - channel allowlist is the real control", () => {
  it("rejects a channel that was not opted into", () => {
    const r = validateInbound(
      envelope({ channel: "graph-api-get-mail" }),
      options(),
    );
    assert.strictEqual(r.ok, false);
  ...
  it("rejects a non-string channel", () => {
    for (const channel of [null, 42, {}, undefined, ["speaking-state"]]) {
      assert.strictEqual(
        validateInbound(envelope({ channel }), options()).ok,
        false,
      );
    }
  });
  ...
  it("accepts either a Set or an array of channels", () => {
    assert.strictEqual(
      validateInbound(
        envelope(),
        options({ acceptedChannels: new Set(["speaking-state"]) }),
      ).ok,
      true,
    );
    assert.strictEqual(
      validateInbound(
        envelope(),
        options({ acceptedChannels: ["speaking-state"] }),
      ).ok,
      true,
    );
  });
  ...
  it("rejects everything when no channels are accepted", () => {
    assert.strictEqual(
      validateInbound(envelope(), options({ acceptedChannels: [] })).ok,
      false,
    );
  });
});
...
describe("Bridge protocol - response correlation", () => {
  const responseOpts = (pending) => options({ pendingIds: new Set(pending) });
  ...
  it("rejects a response with a missing or non-string id", () => {
    for (const id of [undefined, "", null, 42]) {
      const r = validateInbound(
        envelope({ kind: KIND_RESPONSE, id }),
        responseOpts(["r1"]),
      );
      assert.strictEqual(r.ok, false);
    }
...
describe("Bridge protocol - payload validation", () => {
  it("runs the validator registered for the channel", () => {
    const validators = {
      "speaking-state": (p) => typeof p?.speaking === "boolean",
    };

    assert.strictEqual(
      validateInbound(envelope(), options({ validators })).ok,
      true,
    );
    assert.strictEqual(
      validateInbound(
        envelope({ payload: { speaking: "yes" } }),
        options({ validators }),
      ).ok,
      false,
    );
...
describe("Bridge protocol - prototype pollution guards", () => {
  ...
  it("allows ordinary nested payloads", () => {
    ...
    assert.strictEqual(
      validateInbound(envelope({ payload }), options()).ok,
      true,
    );
  });
  ...
  it('rejects the opaque "null" origin used by sandboxed and data: documents', () => {
    for (const origin of ["null", "", undefined, null, 42]) {
      ...
      assert.strictEqual(
        ok,
        false,
        `Expected rejection for origin ${JSON.stringify(origin)}`,
      );
    }
  });
