...
describe("Object message sanitization", () => {
  ...
  test("preserves non-PII object properties", () => {
    const result = sanitizeLogData([
      { count: 42, status: "connected", active: true },
    ]);
...
describe("Teams for Linux realistic scenarios", () => {
  test("MQTT connection log", () => {
    const result = sanitizeLogData([
      "MQTT connecting to:",
      {
        brokerUrl: "mqtt://mqttuser:secretpass@home-mqtt.local:1883",
        clientId: "teams-linux",
      },
    ]);
    assert.ok(
      result[1].brokerUrl.includes("[CREDENTIALS]@"),
      `Expected credentials redacted: ${result[1].brokerUrl}`,
    );
  ...
  test("Graph API error log", () => {
    const result = sanitizeLogData([
      "Graph API error:",
      {
        url: "https://graph.microsoft.com/v1.0/me?$select=mail&access_token=eyJhbGc",
        code: 401,
      },
    ]);
    assert.ok(
      result[1].url.includes("?[PARAMS]"),
      `Expected params redacted: ${result[1].url}`,
    );
  ...
  test("Config file path log", () => {
    ...
    assert.ok(
      result[1].includes("/home/[USER]"),
      `Expected username redacted: ${result[1]}`,
    );
  });
});
...
describe("Error and edge cases", () => {
  ...
  test("handles Error objects with PII", () => {
    ...
    assert.ok(
      result[0].message.includes("[EMAIL]"),
      `Expected email in error message to be sanitized: ${result[0].message}`,
    );
  });
  ...
  test("handles Error objects with custom properties", () => {
    ...
    assert.strictEqual(
      result[0].statusCode,
      403,
      "Numeric properties should be preserved",
    );
    // URL paths with email get sanitized as [PATH], which is correct behavior
    assert.ok(
      result[0].endpoint &&
        (result[0].endpoint.includes("[EMAIL]") ||
          result[0].endpoint.includes("[PATH]")),
      `String properties with PII should be sanitized, got: ${result[0].endpoint}`,
    );
    assert.ok(
      result[0].metadata?.email?.includes("[EMAIL]"),
  ...
  test("sanitizes PII in object keys", () => {
    ...
    assert.ok(
      "[EMAIL]" in result[0],
      `Expected email key to be sanitized: ${JSON.stringify(result[0])}`,
    );
    assert.ok(
      "[IP]" in result[0],
      `Expected IP key to be sanitized: ${JSON.stringify(result[0])}`,
    );
    // Original PII keys should not exist
    assert.ok(
      !("user@example.com" in result[0]),
      "Original email key should not exist",
    );
    assert.ok(
      !("192.168.1.1" in result[0]),
      "Original IP key should not exist",
    );
  });
});
