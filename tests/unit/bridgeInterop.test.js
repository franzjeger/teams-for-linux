...
function createHarness({ sessionId = SESSION, origin = ORIGIN } = {}) {
  ...
  function dispatch(data) {
    ...
    for (const fn of [...agentListeners])
      fn({ data: clone(), source: innerGlobal, origin });
    for (const fn of [...preloadListeners])
      fn({ data: clone(), source: sharedWindow, origin });
  }
...
describe("Bridge interop - real agent against real bridge", () => {
  ...
  it("drops an event on a channel the bridge did not accept", async () => {
    ...
    assert.doesNotThrow(() =>
      agentApi.emit("graph-api-get-mail", { secret: true }),
    );
    await new Promise((r) => setTimeout(r, 5));
  });
  ...
  it("ignores a request forged with the wrong session id", async () => {
    ...
    assert.strictEqual(
      called,
      false,
      "agent must not run capabilities for foreign sessions",
    );
  });
  ...
  it("does not let the agent consume its own responses as requests", async () => {
    ...
    assert.strictEqual(
      invocations,
      1,
      "a response must not loop back as a new request",
    );
  });
