...
test.describe("Integration surface", () => {
  ...
  async function launchReady(testInfo, extraArgs = []) {
    electronApp = await launchAuthenticatedApp(
      testInfo.project.use.sessionDir,
      extraArgs,
    );
    ...
    expect(
      ready,
      "Preload should have installed its page instrumentation",
    ).toBe(true);
    return mainWindow;
  }
  ...
  test("a Notification fires its show event asynchronously", async ({}, testInfo) => {
    ...
    expect(
      fired,
      "Teams relies on the show event to advance its state machine",
    ).toBe(true);
  });
  ...
  test("an enabled camera tool patches getUserMedia in the page world", async ({}, testInfo) => {
    ...
    expect(patched.available, "mediaDevices.getUserMedia should exist").toBe(
      true,
    );
    expect(
      patched.isNative,
  ...
  test("the page-exposed privileged surface stays minimal", async ({}, testInfo) => {
    ...
    expect(
      exposed,
      "electronAPI should exist for the injected screen sharing script",
    ).not.toBeNull();
    expect(exposed).toEqual([
      "sendScreenSharingStarted",
