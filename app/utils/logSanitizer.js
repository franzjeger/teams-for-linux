...
const PII_PATTERNS = {
  ...
  urlQueryParams: /\?[^"'\s]+/g,
  authHeader:
    /authorization[=:]\s*['"]?(?:basic|digest|ntlm|negotiate|hoba|mutual|aws4-hmac-sha256)\b[^'"\n,;]*['"]?/gi,
  ...
  certFingerprint:
    /(?:fingerprint|sha1|sha256)[=:]\s*['"]?[a-f0-9:]{40,}['"]?/gi,
...
function sanitize(message) {
  ...
  sanitized = sanitized.replaceAll(
    PII_PATTERNS.authHeader,
    "Authorization: [REDACTED]",
  );
  sanitized = sanitized.replaceAll(PII_PATTERNS.apiKey, "api_key=[REDACTED]");
  sanitized = sanitized.replaceAll(
    PII_PATTERNS.accessToken,
    "access_token=[REDACTED]",
  );
  sanitized = sanitized.replaceAll(
    PII_PATTERNS.refreshToken,
    "refresh_token=[REDACTED]",
  );
  sanitized = sanitized.replaceAll(
    PII_PATTERNS.clientSecret,
  ...
  sanitized = sanitized.replaceAll(PII_PATTERNS.userPath, (match) => {
    ...
    if (match.toLowerCase().startsWith("c:\\users\\"))
      return "C:\\Users\\[USER]";
    return "[PATH]";
  });
...
const LOWER_CASE_SENSITIVE_KEYS = ["password", "token", "secret", "key"];
