...
class MQTTClient extends EventEmitter {
      ...
      this.client.on("connect", () => {
        ...
        if (this.config.commandTopic) {
          ...
          this.client.subscribe(commandTopic, (err) => {
            if (err) {
              console.error(
                `[MQTT] Failed to subscribe to command topic ${commandTopic}:`,
                err,
              );
            } else {
              console.info(
                `[MQTT] Subscribed to command topic: ${commandTopic}`,
              );
            }
          });
        } else {
          console.debug(
            "[MQTT] Command topic not configured, skipping command subscription",
          );
        }
      });
  ...
  async publish(topic, payload, options = {}) {
    ...
    try {
      ...
      console.debug(
        `[MQTT] Published to ${topic}: ${payloadString.substring(0, 100)}`,
      );
    } catch (error) {
  ...
  async publishStatus(status) {
    ...
    try {
      ...
      console.debug(
        `[MQTT] Published status: ${statusString} (${status}) on topic: ${topic}`,
      );
    } catch (error) {
  ...
  handleCommand(messageString) {
    try {
      ...
      if (!this.allowedActions.includes(command.action)) {
        console.warn(
          `[MQTT] Invalid command: action '${command.action}' not in whitelist`,
        );
        return;
      }
