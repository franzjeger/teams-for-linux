...
class IdleMonitor {
  ...
  constructor(config, getUserStatus) {
    ...
    const stateFilePath =
      config.idleDetection?.stateFile ||
      "/tmp/teams-for-linux-idle-state-$USER";
    this.#stateFilePath = stateFilePath.replace(
      "$USER",
  ...
  initialize() {
    ...
    ipcMain.handle(
      ...
      this.#handleGetSystemIdleState.bind(this),
    );
    ...
    try {
      if (fs.existsSync(this.#stateFilePath)) {
        ...
        if (content === IdleMonitor.#STATE_FILE_INACTIVE) {
          ...
          console.warn(
            `[IDLE] Unknown state file content: '${content}', ignoring`,
          );
          return null;
        }
      }
  ...
  #handleStateFileOverride() {
    ...
    if (stateFileOverride !== this.#lastStateFileOverride) {
      if (stateFileOverride === null) {
        console.info(
          "[IDLE] State file override: none (file absent or invalid)",
        );
      } else {
    ...
    if (stateFileOverride === IdleMonitor.#SYSTEM_STATE_IDLE) {
      ...
      if (this.#idleTimeUserStatus !== -1) {
        console.debug(
          `[IDLE] State file active: transitioning from idle to active`,
        );
  ...
  async #handleGetSystemIdleState() {
    ...
    const logDetails = () =>
      `IdleTimeout: ${this.#config.appIdleTimeout}s, IdleTimeoutPollInterval: ${
