...
function registerGraphApiHandlers(ipcMain, graphApiClient) {
  ...
  ipcMain.handle("graph-api-get-user-profile", async () => {
    ...
    try {
      ...
      logger.error("[GRAPH_API] getUserProfile failed:", {
  ...
  ipcMain.handle("graph-api-get-calendar-events", async (_event, options) => {
    ...
    try {
      ...
      logger.error("[GRAPH_API] getCalendarEvents failed:", {
        message: error.message,
      });
      return { success: false, error: error.message };
    }
  ...
  ipcMain.handle(
    ...
    async (_event, startDateTime, endDateTime, options) => {
      if (!graphApiClient) return notEnabled;
      try {
        return await graphApiClient.getCalendarView(
          startDateTime,
          endDateTime,
          options,
        );
      } catch (error) {
        logger.error("[GRAPH_API] getCalendarView failed:", {
          message: error.message,
        });
        return { success: false, error: error.message };
      }
  ...
  ipcMain.handle("graph-api-create-calendar-event", async (_event, event) => {
    ...
    try {
      ...
      logger.error("[GRAPH_API] createCalendarEvent failed:", {
        message: error.message,
      });
      return { success: false, error: error.message };
    }
  ...
  ipcMain.handle("graph-api-get-mail-messages", async (_event, options) => {
    ...
    try {
      ...
      logger.error("[GRAPH_API] getMailMessages failed:", {
        message: error.message,
      });
      return { success: false, error: error.message };
    }
  ...
  ipcMain.handle("graph-api-search-people", async (_event, query, options) => {
    ...
    try {
      ...
      logger.error("[GRAPH_API] searchPeople failed:", {
        message: error.message,
      });
      return { success: false, error: error.message };
    }
  ...
  ipcMain.handle(
    ...
    async (_event, contactInfo, content) => {
      if (!graphApiClient) return notEnabled;
      if (
        !contactInfo ||
        typeof contactInfo.userId !== "string" ||
        !contactInfo.userId.trim()
      ) {
        return {
          success: false,
          error: "Invalid contact info: userId required",
        };
      }
      ...
      try {
        return await graphApiClient.sendChatMessageToUser(
          contactInfo,
          content.trim(),
        );
      } catch (error) {
        logger.error("[GRAPH_API] sendChatMessage failed:", {
          message: error.message,
        });
        return { success: false, error: error.message };
      }
