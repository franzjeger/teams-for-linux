...
class GraphApiClient {
  ...
  async getCalendarView(startDateTime, endDateTime, options = {}) {
    logger.debug("[GRAPH_API] Getting calendar view", {
      startDateTime,
      endDateTime,
    });
  ...
  async createCalendarEvent(event) {
    logger.debug("[GRAPH_API] Creating calendar event", {
  ...
  async resolveConversation(recipientUserId) {
    try {
      ...
      const candidates = await this.mainWindow.webContents
      ...
      logger.info("[GRAPH_API] Found candidate chat IDs", {
      ...
      for (const chatId of candidates) {
        const membersResult = await this.makeRequest(
      ...
      return {
        success: false,
        error:
          "Could not find a chat with this user. You may need to start a conversation in Teams first.",
      };
    } catch (error) {
  ...
  async sendChatMessageToUser(contactInfo, content) {
    try {
      ...
      const conversationResult = await this.resolveConversation(
        contactInfo.userId,
      );
