...
class TimestampCopyOverride {
  ...
  applyOverride() {
    ...
    if (
      coreSettings.get("compose").disableTimestampOnCopy ===
      this.config.disableTimestampOnCopy
    ) {
      console.debug(
        "Setting disableTimestampOnCopy is correct, stopping polling",
      );
      this.stop();
    }
    ...
    const composeSubject =
      coreSettings.categoryBehaviorSubjectMap.get("compose");
