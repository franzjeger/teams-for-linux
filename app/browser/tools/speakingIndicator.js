...
class SpeakingIndicator {
  ...
  #onPeerConnectionCreated(pc) {
    ...
    console.info(
      `${LOG_PREFIX} RTCPeerConnection created (total: ${this.#peerConnections.length})`,
    );
  ...
  #registerCallEvents() {
    ...
    activityHub.on("call-disconnected", () => {
      console.info(
        `${LOG_PREFIX} call-disconnected event received, clearing connections`,
      );
      ...
      for (const pc of this.#peerConnections) {
        ...
        report.forEach((stat) => {
          ...
          if (newState !== this.#state) {
            console.info(
              `${LOG_PREFIX} State: ${this.#state} → ${newState} (audioLevel=${level.toFixed(5)})`,
            );
            this.#state = newState;
