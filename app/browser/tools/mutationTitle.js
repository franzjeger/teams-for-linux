class MutationObserverTitle {
  ...
  _applyMutationToTitleLogic() {
    ...
    try {
      ...
      const observer = new globalThis.MutationObserver(() => {
        try {
          ...
          if (number === this.#lastNumber) {
            console.debug(
              `MutationTitle: Number unchanged (${number}), skipping event dispatch`,
            );
            return;
          }
          ...
          if (Number.isNaN(number) || number < 0 || number > 9999) {
            console.warn(
              "MutationTitle: Invalid unread count extracted:",
              number,
            );
            return;
          }
          ...
          console.debug(
            `MutationTitle: Dispatching unread-count event with number: ${number}`,
          );
      ...
      console.debug(
        "MutationTitle: Observer successfully attached to title element",
      );
    } catch (error) {
      console.error(
        "MutationTitle: Error setting up mutation observer:",
        error,
      );
    }
  }
