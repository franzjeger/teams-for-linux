...
class MQTTStatusMonitor {
  ...
  setupMutationObserver() {
    ...
    console.debug(
      "Mutation observer set up for status monitoring (with 300ms debounce)",
    );
  }
  ...
  detectCurrentStatus() {
    ...
    if (status !== null) {
      if (!this._loggedDetection)
        console.info("[MQTT Status] Detected via CSS selectors:", status);
    ...
    if (status !== null) {
      if (!this._loggedDetection)
        console.info("[MQTT Status] Detected via me-control:", status);
      this._loggedDetection = true;
      return status;
    }
    ...
    if (status !== null) {
      if (!this._loggedDetection)
        console.info("[MQTT Status] Detected via page title:", status);
      this._loggedDetection = true;
      return status;
    }
  ...
  detectStatusFromMeControl() {
    ...
    const meControl =
      document.querySelector('[data-tid="me-control-avatar-trigger"]') ||
    ...
    const presenceIndicator =
      meControl.querySelector('[class*="presence" i]') ||
  ...
  extractStatusFromElement(element) {
    ...
    const presenceChild = element.querySelector(
      '[class*="presence"], [data-tid*="presence"]',
    );
    if (presenceChild) {
      childPresenceInfo =
        presenceChild.classList.toString() +
        " " +
        (presenceChild.getAttribute("aria-label") || "");
    }

    return this.mapTextToStatusCode(
      classList,
      ariaLabel,
      title,
      textContent,
